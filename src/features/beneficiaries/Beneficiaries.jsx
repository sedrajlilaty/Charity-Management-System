...
export const CAT_CFG = {
  patient: {
    ...
    labelKey: 'beneficiaries.categories.patient',
  },
  ...
  orphan: {
    ...
    labelKey: 'beneficiaries.categories.orphan',
  },
  ...
  school_student: {
    ...
    color: '#92400e',
    bg: '#fef3c7',
    labelKey: 'beneficiaries.categories.school_student',
  },
  ...
  university_student: {
    ...
    color: '#92400e',
    bg: '#fef3c7',
    labelKey: 'beneficiaries.categories.university_student',
  },
}
...
export default function Beneficiaries() {
  ...
  const handleSave = async (form) => {
    try {
      ...
      const base = {
        ...
        personal_picture:
          form.personal_picture instanceof File
      ...
      toast.success(
        t(
          'beneficiaries.toast.createSuccess',
          {
            defaultValue: 'تمت إضافة الطلب بنجاح ✅',
          }
        )
      )
      ...
      toast.error(
        ...
        t(
          'beneficiaries.toast.createError',
          {
            defaultValue: 'فشل إضافة الطلب',
          }
        )
      )
  ...
  const handleApproveAndPublish = async (enrichedData) => {
    try {
      await acceptMut.mutateAsync({
        ...
        title:
          enrichedData.caseTitle ??
          enrichedData.title,

        description:
          enrichedData.caseDescription ??
          enrichedData.description,

        required_amount:
          enrichedData.required_amount,

        personal_picture:
          enrichedData.personal_picture instanceof File
            ? enrichedData.personal_picture
            : undefined,
      })

      toast.success(
        t(
          'beneficiaries.toast.acceptSuccess',
          {
            defaultValue: 'تم قبول الطلب ونشره ✅',
          }
        )
      )
      ...
      const msg =
        ...
        t(
          'beneficiaries.toast.acceptError',
          {
            defaultValue: 'فشل قبول الطلب',
          }
        )
  ...
  const handleClose = (row) => {
    closeMut.mutate(row.id, {
      onSuccess: () =>
        toast.success(
          t(
            'beneficiaries.toast.closeSuccess',
            {
              defaultValue: 'تم إغلاق الطلب',
            }
          )
        ),
    ...
    rejectMut.mutate(
      ...
      },
      {
        onSuccess: () =>
          toast.success(
            t(
              'beneficiaries.toast.rejectSuccess',
              {
                defaultValue: 'تم رفض الطلب',
              }
            )
          ),
  ...
  const handleAction = (action, row) => {
    switch (action) {
      ...
      case 'view':
        setViewItem(row)
        setViewInitStep('view')
        setCaseViewOpen(true)
        break

      case 'approve':
        setViewItem(row)
        setViewInitStep('publish')
        setCaseViewOpen(true)
        break
  ...
  const STATUS_TABS = [
    {
      key: '',
      label: t('beneficiaries.tabs.all'),
    },

    {
      key: 'accepted',
      label: t('beneficiaries.tabs.active'),
    },

    {
      key: 'pending',
      label: t('beneficiaries.tabs.pending'),
    },

    {
      key: 'rejected',
      label: t('beneficiaries.tabs.rejected'),
    },
  ]
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  {t(
                    'beneficiaries.categories.label'
                  )}
                  :
                </span>
                ...
                >
                  {t(
                    'beneficiaries.categories.all'
                  )}
                </button>


                {Object.entries(CAT_CFG).map(
                  ([key, cfg]) => {
